...
export default function SalesView({ salesTargets }: Props) {
  ...
  const barOption = {
    tooltip: { trigger: 'axis', backgroundColor: '#fff', borderColor: COLORS.border, borderWidth: 1, borderRadius: 8 },
    grid: { left: 60, right: 20, top: 40, bottom: 40 },
    xAxis: {
      type: 'category', data: salesTargets.map((s) => s.name),
      axisLine: { lineStyle: { color: COLORS.border } }, axisLabel: { color: COLORS.textSecondary },
    },
    yAxis: {
      type: 'value', max: 120, axisLine: { show: false },
      axisLabel: { formatter: '{value}%', color: COLORS.textTertiary },
      splitLine: { lineStyle: { color: COLORS.border, type: 'dashed' } },
    },
    series: [{
      name: '目标完成率', type: 'bar', barWidth: 40,
      ...
      data: salesTargets.map((s) => {
        ...
        return { value: rate, itemStyle: { color: completionColor(rate), borderRadius: [6, 6, 0, 0] } }
      }),
    }],
  }
  ...
  const profitOption = {
    ...
    series: [
      {
        name: '计划利润率', type: 'bar', barWidth: 20,
        data: salesTargets.map(() => 25),
        itemStyle: { color: COLORS.border, borderRadius: [4, 4, 0, 0] },
      },
      {
        name: '实际利润率', type: 'bar', barWidth: 20,
  ...
  const columns = [
    ...
    {
      title: '计划收入', dataIndex: 'planRevenue', width: 120, align: 'right' as const,
      render: (v: number) => fmtAmountShort(v),
    },
    {
      title: '实际收入', dataIndex: 'actualRevenue', width: 120, align: 'right' as const,
      ...
      render: (_: unknown, record: SalesTarget) => {
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Progress percent={Math.min(rate, 100)} size="small" showInfo={false}
              stroke={completionColor(rate)} style={{ flex: 1, height: 8, borderRadius: 4 }} />
            <span style={{ fontSize: 12, fontWeight: 600, color: completionColor(rate), minWidth: 40 }}>{rate}%</span>
          </div>
        )
      },
    },
    {
      title: '利润率', width: 140,
      render: (_: unknown, record: SalesTarget) => {
        const isGood = record.actualProfitRate >= 25
        return (
          <div style={{
            ...
            <span style={{ fontSize: 11, color: COLORS.textTertiary, marginLeft: 4 }}>目标25%</span>
    ...
    {
      title: '项目数', dataIndex: 'projectCount', width: 100, align: 'center' as const,
      render: (v: number) => (
        <span style={{ fontWeight: 600, color: COLORS.primary, cursor: 'pointer' }}>{v} 个</span>
      ),
    },
  ]
